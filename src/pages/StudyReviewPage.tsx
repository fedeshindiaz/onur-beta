...
const fieldClass = 'h-10 min-w-28 rounded-xl border border-[#E9E7E7] bg-white px-3 text-xs text-[#171717]'
...
export function StudyReviewPage() {
  ...
  if (isPending || extractionPending) return <p className="text-sm text-[#747474]">Cargando estudio…</p>
  ...
  if (extraction && extraction.status !== 'manual') return <div className="space-y-7">
    <Link to={`/app/pacientes/${study.patientId}`} className="inline-flex items-center gap-2 text-xs font-black text-[#E49A02]"><ChevronLeft size={16}/> Volver al paciente</Link>
  ...
  return <div className="space-y-7">
    ...
    {study.calculationMethodVersion.startsWith('onur-bap-webserial-')&&<div className="flex gap-3 rounded-2xl border border-[#E8CE99] bg-[#FFF7E8] p-5"><Activity className="shrink-0 text-[#A36B00]" size={20}/><div><p className="text-sm font-black text-[#7A5100]">Parámetros recibidos directamente del BAP</p><p className="mt-1 text-xs leading-5 text-[#6F5A2A]">Se registraron métricas calculadas, no tramas crudas. Verificá condiciones, seguridad y calidad antes de indicar que el estudio es interpretable o finalizarlo.</p></div></div>}
    {study.status==='finalized'&&<div className="flex gap-3 rounded-2xl border border-[#E8CE99] bg-[#FFF7E8] p-5"><LockKeyhole className="shrink-0 text-[#A36B00]" size={20}/><div><p className="text-sm font-black text-[#7A5100]">Estudio finalizado</p><p className="mt-1 text-xs leading-5 text-[#6F5A2A]">La revisión quedó bloqueada para preservar el registro confirmado. Las sugerencias profesionales pueden seguir revisándose por separado.</p></div></div>}
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <article className="rounded-2xl border border-[#E9E7E7] bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#747474]">Protocolo</p><p className="mt-2 text-sm font-black text-[#2F2F2F]">{study.protocolCode} · v{study.protocolVersion}</p><p className="mt-1 text-xs text-[#747474]">{study.deviceName || 'Equipo no informado'}</p></article>
      <article className="rounded-2xl border border-[#E9E7E7] bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#747474]">Valores utilizables</p><p className="mt-2 text-2xl font-black text-[#27734c]">{counts.ok}</p><p className="mt-1 text-xs text-[#747474]">Sin incidencias abiertas</p></article>
      <article className="rounded-2xl border border-[#E9E7E7] bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#747474]">A revisar</p><p className="mt-2 text-2xl font-black text-[#8A5B00]">{counts.review}</p><p className="mt-1 text-xs text-[#747474]">Requieren confirmación</p></article>
      <article className="rounded-2xl border border-[#E9E7E7] bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#747474]">Bloqueados / cuarentena</p><p className="mt-2 text-2xl font-black text-[#a94952]">{counts.blocked + counts.quarantine}</p><p className="mt-1 text-xs text-[#747474]">No participan en sugerencias</p></article>
    </section>
    ...
    <section className="rounded-2xl border border-[#E9E7E7] bg-white">
      <div className="flex flex-col gap-4 border-b border-[#E9E7E7] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div><h2 className="text-lg font-black text-[#171717]">Valor original y normalizado</h2><p className="mt-1 text-xs leading-5 text-[#747474]">El original nunca se sobrescribe. La normalización usa la regla onur-normalization-1.0.</p></div>
        <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setPasteOpen((value) => !value)} className="inline-flex items-center gap-2 rounded-2xl border border-[#E9E7E7] px-4 py-2.5 text-xs font-black text-[#2F2F2F]"><ClipboardPaste size={15}/> Pegar tabla</button><button type="button" onClick={() => {setRows((current) => [...current, emptyMetric()]);setDirty(true)}} className="inline-flex items-center gap-2 rounded-2xl bg-[#E49A02] px-4 py-2.5 text-xs font-black text-white"><Plus size={15}/> Agregar fila</button></div>
      </div>

      {pasteOpen && <div className="border-b border-[#E9E7E7] bg-[#F7F6F4] p-5 sm:p-6"><label className="text-xs font-black text-[#2F2F2F]">Tabla CSV, punto y coma o tabulaciones<textarea value={pasteText} onChange={(event) => setPasteText(event.target.value)} className="mt-2 min-h-32 w-full rounded-2xl border border-[#E9E7E7] bg-white p-4 font-mono text-xs" placeholder={'metrica;valor;unidad;condicion;lado;eje;repeticion;origen\ncondition_score;82,5;percent;A;;;1;Página 1'}/></label><div className="mt-3 flex justify-end"><button type="button" onClick={importTable} className="rounded-2xl bg-[#171717] px-4 py-2.5 text-xs font-black text-white">Importar filas</button></div></div>}

      <div className="overflow-x-auto">
        <table className="w-full min-w-[1280px] text-left">
          <thead className="bg-[#F7F6F4] text-[10px] font-black uppercase tracking-[.12em] text-[#747474]"><tr><th className="px-4 py-3">Métrica</th><th className="px-3 py-3">Valor original</th><th className="px-3 py-3">Unidad</th><th className="px-3 py-3">Condición</th><th className="px-3 py-3">Lado</th><th className="px-3 py-3">Eje</th><th className="px-3 py-3">Rep.</th><th className="px-3 py-3">Ubicación</th><th className="px-3 py-3">Normalizado / calidad</th><th className="px-3 py-3"></th></tr></thead>
          <tbody className="divide-y divide-[#E9E7E7]">{normalized.map((row) => <tr key={row.clientId} className="align-top">
            ...
            <td className="p-3"><div className="w-64"><div className="flex items-center gap-2"><StatusBadge status={row.qualityStatus}/><span className="text-xs font-black text-[#2F2F2F]">{row.normalizedNumericValue ?? row.normalizedTextValue ?? '—'}</span></div><p className="mt-2 text-[11px] font-bold text-[#747474]">{metricLabel(row.metricCode)}</p>{row.issues.map((issue, index) => <p key={`${issue.ruleCode}-${index}`} className="mt-1 text-[11px] leading-4 text-[#8A5B00]"><strong>{issue.ruleCode}:</strong> {issue.message}</p>)}</div></td>
    ...
    <section className="grid gap-5 lg:grid-cols-[1fr_.72fr]">
      <article className="rounded-2xl border border-[#E9E7E7] bg-white p-6"><h2 className="text-lg font-black text-[#171717]">Confirmación profesional</h2><label className="mt-5 block text-sm font-black text-[#2F2F2F]">Observaciones técnicas<textarea className="mt-2 min-h-28 w-full rounded-2xl border border-[#E9E7E7] p-4 text-sm font-normal" value={qualityNotes} onChange={(event) => {setQualityNotes(event.target.value);setDirty(true)}} placeholder="Condiciones de realización, asistencia, sensor, fatiga u otras limitaciones."/></label><label className={`mt-4 flex items-start gap-3 rounded-2xl p-4 text-sm font-bold ${hasBlockingValues ? 'bg-[#f2f3f3] text-[#747474]' : 'bg-[#F7F6F4] text-[#2F2F2F]'}`}><input type="checkbox" className="mt-1" checked={interpretable && !hasBlockingValues} disabled={hasBlockingValues} onChange={(event) => {setInterpretable(event.target.checked);setDirty(true)}}/><span>Considero que el estudio es interpretable<small className="mt-1 block font-normal">Esta decisión pertenece al profesional. La app solo impide marcarlo cuando hay valores bloqueados o en cuarentena.</small></span></label></article>
      <aside className="space-y-4"><div className="rounded-2xl border border-[#E9E7E7] bg-white p-6"><h2 className="font-black text-[#171717]">Resumen antes de confirmar</h2><dl className="mt-4 space-y-3 text-sm"><div className="flex justify-between"><dt className="text-[#747474]">Métricas</dt><dd className="font-black">{normalized.length}</dd></div><div className="flex justify-between"><dt className="text-[#747474]">Incidencias</dt><dd className="font-black">{counts.issues}</dd></div><div className="flex justify-between"><dt className="text-[#747474]">No aplica</dt><dd className="font-black">{counts.notApplicable}</dd></div></dl></div>{hasBlockingValues && <div className="flex gap-3 rounded-2xl border border-[#E8CE99] bg-[#FFF7E8] p-5"><AlertTriangle className="shrink-0 text-[#8A5B00]" size={20}/><p className="text-xs leading-5 text-[#8A5B00]">Podés conservar la importación, pero los valores bloqueados o en cuarentena no se usarán en sugerencias ni comparaciones.</p></div>}{notice && <div className="flex gap-3 rounded-2xl border border-[#E8CE99] bg-[#FFF7E8] p-5"><CheckCircle2 className="shrink-0 text-[#A36B00]" size={20}/><p className="text-xs font-bold leading-5 text-[#A36B00]">{notice}</p></div>}{error && <p role="alert" className="rounded-2xl bg-[#fceced] p-4 text-sm font-bold text-[#a94952]">{error}</p>}<button type="button" disabled={save.isPending || study.status === 'finalized'} onClick={confirm} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#E49A02] px-5 py-4 text-sm font-black text-white disabled:opacity-50"><Save size={17}/>{save.isPending ? 'Confirmando…' : 'Confirmar importación'}</button>{study.status==='reviewed'&&<button type="button" disabled={dirty||finalize.isPending} onClick={finalizeReview} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#E8CE99] bg-white px-5 py-4 text-sm font-black text-[#E49A02] disabled:opacity-45"><LockKeyhole size={17}/>{finalize.isPending?'Finalizando…':dirty?'Confirmá los cambios antes de finalizar':'Finalizar y bloquear estudio'}</button>}</aside>
    </section>
    </fieldset>
  </div>
}
