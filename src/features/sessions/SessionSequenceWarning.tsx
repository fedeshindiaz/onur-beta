  ...
  return <aside className="mb-5 rounded-2xl border border-[#E8CE99] bg-[#FFF7E8] p-5 text-[#8A5B00]">
    <div className="flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {!analysis.optimizedForVrBox && <button type="button" onClick={() => onReorder(orderExercisesForVrBox(exercises))} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#8A5B00] px-4 py-3 text-xs font-black text-white"><WandSparkles size={15}/> Ordenar repeticiones antes del visor</button>}
      </div>
    </div>
  </aside>
}
