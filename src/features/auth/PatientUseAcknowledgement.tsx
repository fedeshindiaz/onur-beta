...
export function PatientUseAcknowledgement() {
  ...
  return <article className="mt-8 rounded-[32px] border border-[#bcded9] bg-white p-7 shadow-[0_20px_48px_rgba(18,50,56,.08)] sm:p-9">
    ...
    <button type="button" disabled={accept.isPending} onClick={()=>accept.mutate()} className="mt-7 h-14 w-full rounded-2xl bg-[#0b7a75] text-sm font-black text-white disabled:opacity-60">{accept.isPending?'Guardando…':'Entiendo y continuar'}</button>
    <p className="mt-4 text-center text-[10px] text-[#7d9094]">Versión {PATIENT_ACKNOWLEDGEMENT.version} · Se guardará fecha y cuenta de aceptación.</p>
  </article>
}
